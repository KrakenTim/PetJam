...
namespace StylizedWater2
{
    public class MaterialUI : ShaderGUI
    {
        ...
        private bool enableLightReflection;
        ...
        public void FindProperties(MaterialProperty[] props)
        {
            ...
            enableLightReflection = !targetMat.IsKeywordEnabled("_SPECULARHIGHLIGHTS_OFF");
            #if URP
        ...
        public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
        {
            ...
            UI.DrawNotification(materialEditor.targets.Length > 1, "Multi-editing of materials is not supported.\n\nThis will be added in version 1.2.0 but will introduce some breaking changes", MessageType.Warning);
        ...
        private void ApplyChanges()
        {
            ...
            CoreUtils.SetKeyword(targetMat, "_SPECULARHIGHLIGHTS_OFF", !enableLightReflection);
        ...
        private void DrawGeneral()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(generalSection.anim.faded))
            {
                ...
                cullingMode = EditorGUILayout.Popup(new GUIContent("Culling", "Controls which sides of the water mesh are visible"), cullingMode, new string[] { "Double-sided", "Front-faces", "Back-faces" });
                ...
                {
                    uvMode = EditorGUILayout.Popup(new GUIContent("UV Source", "Use either the mesh's UV or world-space units as a base for texture tiling"), uvMode, new string[] {"Mesh UV", "World XZ projected"});
                }
                ...
                {
                    EditorGUI.indentLevel++;
                    materialEditor.ShaderProperty(_SlopeThreshold, new GUIContent(_SlopeThreshold.displayName, "A higher value results in foam also drawing on surfaces that are relatively flat"));
        ...
        private void DrawLighting()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(lightingSection.anim.faded))
            {
                ...
                enableFlatShading = UI.Material.Toggle(enableFlatShading, "Flat/low-poly shading", "When enabled, normals are calculated per mesh face, resulting in a faceted appearance (low poly look)");
                ...
                using (new EditorGUI.DisabledScope(!enableLighting))
                {
                    if (enableShadows)
                    {
                        EditorGUI.indentLevel++;
                ...
                if (enableTranslucency)
                {
                    ...
                    materialEditor.ShaderProperty(_TranslucencyReflectionMask, new GUIContent("Reflection Mask", "Controls how strongly reflections are laid over the effect. A value of 1 is physically accurate"));
                    EditorGUI.indentLevel--;
                }
        ...
        private void DrawColor()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(colorSection.anim.faded))
            {
                ...
                using (new EditorGUI.DisabledGroupScope(_DisableDepthTexture.floatValue == 1f))
                {
                    ...
                    UI.Material.DrawSlider(_DepthVertical, label:"Distance Depth", tooltip:"Distance measured from the camera to the surface behind the water. Water turns denser the more the camera looks along it");
                    UI.Material.DrawSlider(_DepthHorizontal, label:"Vertical Depth");
                ...
                enableOpacityVC = UI.Material.Toggle(enableOpacityVC, "Vertex color opacity (G)", "The Green vertex color channel also adds opacity");
                using (new EditorGUI.DisabledGroupScope(_DisableDepthTexture.floatValue == 1f))
                {
        ...
        private void DrawNormals()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(normalsSection.anim.faded))
            {
                ...
                if (_NormalMapOn.floatValue == 1f)
                {
                    ...
                    UI.Material.DrawSlider(_NormalStrength, "Strength (lighting)");
                    if (!enableLighting)
                    {
        ...
        private void DrawFoam()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(foamSection.anim.faded))
            {
                ...
                if (enableFoam)
                {
                    ...
                    enableFoamVC = UI.Material.Toggle(enableFoamVC, "Vertex color painting (A)",
        ...
        private void DrawReflections()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(reflectionSection.anim.faded))
            {
                ...
                enableLightReflection = EditorGUILayout.Toggle("Enable", enableLightReflection);
                if (enableLightReflection)
                {
                    EditorGUILayout.Space();

                    UI.Material.DrawFloatField(_SunReflectionStrength, tooltip:"This value is multiplied over the sun light's intensity");
                    materialEditor.ShaderProperty(_SunReflectionSize, _SunReflectionSize.displayName);
                    materialEditor.ShaderProperty(_SunReflectionDistortion, new GUIContent(_SunReflectionDistortion.displayName, "Note: Distortion is largely influenced by the strength of the normal map texture and wave curvature"));

                    if (enableLighting)
                    {
                        EditorGUILayout.Space();
                        materialEditor.ShaderProperty(_PointSpotLightReflectionExp, new GUIContent(_PointSpotLightReflectionExp.displayName, "Specular reflection size for point/spot lights"));
                    }
                }
                ...
                #endif
                if (enableEnvReflections && RenderSettings.defaultReflectionMode == DefaultReflectionMode.Custom && !customReflection)
                {
            ...
            if (EditorGUILayout.BeginFadeGroup(wavesSection.anim.faded))
            {
                ...
                using (new EditorGUI.DisabledGroupScope(enableRiverMode))
                {
                    ...
                    if (_WavesOn.floatValue == 1 && !enableRiverMode)
                    {
                        ...
                        enableWaveVC = UI.Material.Toggle(enableWaveVC, "Vertex color flattening (B)",
        ...
        private void DrawAdvanced()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(advancedSection.anim.faded))
            {
                ...
                if (targetMat.renderQueue <= 2450 || targetMat.renderQueue >= 3500)
                {
